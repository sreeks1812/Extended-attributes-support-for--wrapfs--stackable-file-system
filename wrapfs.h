...
static inline void wrapfs_get_lower_path(const struct dentry *dent,
					 struct path *lower_path)
{
	//mutex_lock_nested(&WRAPFS_D(dent)->lock,WRAPFS_DMUTEX_NORMAL);
	spin_lock(&WRAPFS_D(dent)->lock);
	pathcpy(lower_path, &WRAPFS_D(dent)->lower_path);
	path_get(lower_path);
	//mutex_unlock(&WRAPFS_D(dent)->lock);
...
extern void *wrapfs_xattr_alloc(size_t size, size_t limit);
static inline void wrapfs_xattr_kfree(const void *p)
{
	kfree(p);
}
extern ssize_t wrapfs_getxattr(struct dentry *dentry, const char *name,
				void *value, size_t size);
extern int wrapfs_removexattr(struct dentry *dentry, const char *name);
extern ssize_t wrapfs_listxattr(struct dentry *dentry, char *list,
				 size_t size);
...
//Commenting getting error bcoz of missing struct in super blocks 
/*static inline void wrapfs_read_lock(struct super_block *sb)
{
	if (WRAPFS_SB(sb)->write_lock_owner &&
	    WRAPFS_SB(sb)->write_lock_owner == current->pid)
		return;
	down_read_nested(&WRAPFS_SB(sb)->rwsem, subclass);
}


static inline void wrapfs_read_unlock(struct super_block *sb)
{
	if (WRAPFS_SB(sb)->write_lock_owner &&
	    WRAPFS_SB(sb)->write_lock_owner == current->pid)
		return;
	up_read(&WRAPFS_SB(sb)->rwsem);
}*/



/* Extended attribute - super block locks */

/* Extended attribute - dentry locks */



static inline struct dentry *wrapfs_lock_parent(struct dentry *d)
{
	struct dentry *p;
	...
	p = dget_parent(d);
	if (p != d)
		spin_lock(&WRAPFS_D(p)->lock);
	return p;
}

static inline void wrapfs_unlock_parent(struct dentry *d, struct dentry *p)
{
	BUG_ON(!d);
	BUG_ON(!p);
	if (p != d) {
		BUG_ON(!spin_is_locked(&WRAPFS_D(p)->lock));
		spin_unlock(&WRAPFS_D(p)->lock);
	}
	dput(p);
}

static inline void wrapfs_lock_dentry(struct dentry *d)
{
	BUG_ON(!d);
	spin_lock(&WRAPFS_D(d)->lock);
}

static inline void wrapfs_unlock_dentry(struct dentry *d)
{
	BUG_ON(!d);
	spin_unlock(&WRAPFS_D(d)->lock);
}

/* Extended attribute - dentry locks */

/* Extended attribute - lower dentry */
...
static inline struct dentry *wrapfs_lower_dentry(const struct dentry *dent)
{
	...
	BUG_ON(!dent);
	wrapfs_get_lower_path(dent, &lower_path);
	return lower_path.dentry;
...
#define wrapfs_check_dentry(d)		do { } while (0)
